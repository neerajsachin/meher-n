...
//var tax_payment_id =  460095;
	...
	query = 	'select tax_payment_id, registration_no,apply_dt, receipt_no,issue_dt , from_date, to_date, amount from atms.tblTaxPayment where apply_dt 	>= ? and  apply_dt <= ? order by registration_no asc';
	}
	if(criteria == 'IssueDate'){
	query = 'select tax_payment_id, registration_no,apply_dt, receipt_no,issue_dt ,from_date, to_date, amount from atms.tblTaxPayment where issue_dt 	>= ? and  issue_dt <= ? order by registration_no asc';	
	}
