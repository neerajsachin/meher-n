...
var toDate = new Date(req.params.toDate);
	...
	query = 	'select fitness_cert_id, registration_no,apply_dt, receipt_no,issue_dt , to_date, amount, prev_exp_dt from atms.tblFitnessCert where apply_dt 	>= ? and  apply_dt <= ? order by registration_no asc';
	}
	if(criteria == 'IssueDate'){
	query = 'select fitness_cert_id, registration_no,apply_dt, receipt_no,issue_dt , to_date, amount , prev_exp_dt from atms.tblFitnessCert where issue_dt 	>= ? and  issue_dt <= ? order by registration_no asc';	
	}
...
fitnessCert.prev_exp_dt = (fitnessCert.prev_exp_dt )? new Date(fitnessCert.prev_exp_dt) : null ;
//0709.1
